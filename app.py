...
Uses actual protobuf files – fully Vercel compatible
"""
import hmac
import hashlib
import aiohttp
...
try:
    ...
    print(f"Proto import error: {e} – using fallback dummies")
    # Fallback dummies (only if files missing)
    class DummyMajorLogin:
        def __init__(self):
            ...
            self.memory_available = type('obj', (), {'version': 0, 'hidden_value': 0})()
...
# =============================================================================
# REGIONAL IP BLOCKS & CONFIGURATIONS (unchanged)
# =============================================================================
REGION_IP_RANGES = {
...
def get_rotated_proxy(region):
    try:
        for path in ["proxies.json", os.path.join(os.path.dirname(__file__), "proxies.json")]:
...
def generate_rotated_ip(region):
    ...
    return f"{random.choice(blocks)}{random.randint(1, 254)}"
...
def generate_exponent_number():
    ...
    return ''.join(digits[d] for d in f"{random.randint(1,99999):05d}")
...
# =============================================================================
# ACTIVATION FUNCTIONS (using real protos)
# =============================================================================
def major_login_safe(access_token, open_id, region, client_ip, proxy_url=None):
    ...
    try:
        ...
        resp = requests.post(cfg['major_login_url'], headers=headers, data=payload, verify=False, timeout=10, proxies=proxies)
        if resp.status_code == 200:
            return resp.content
    except Exception as e:
        print(f"major_login_safe error: {e}")
    return None
...
def activate_account(uid, password, region, proxy_url=None):
    try:
        ...
        print(f"activate_account error: {e}")
        return False

# =============================================================================
# ASYNC GENERATION ENGINE (using real protos)
# =============================================================================
async def _perform_major_login_async_api(session, uid, password, access_token, open_id, region, is_ghost, client_ip, proxy_url=None):
    ...
    try:
        ...
        print(f"_perform_major_login_async_api error: {e}")
    return {"account_id": "N/A", "jwt_token": ""}
...
async def _major_register_and_login_async_api(session, uid, password, access_token, open_id, name, region, is_ghost, client_ip, proxy_url=None):
    try:
        ...
        print(f"_major_register_and_login_async_api error: {e}")
        return None
    ...
    async with semaphore:
        for attempt in range(5):
            try:
                ...
                print(f"create_acc_for_api attempt {attempt} error: {e}")
                continue
    return None